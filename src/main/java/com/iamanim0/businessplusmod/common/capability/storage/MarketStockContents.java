...
import com.iamanim0.businessplusmod.common.capability.storage.MarketStockContents.Notify;
...
public class MarketStockContents implements IInventory {
    ...
    public MarketStockContents(int size, Predicate<PlayerEntity> canPlayerAccess, Notify notify) {
        this.vendingComponentContents = new MarketStackHandler(size);
        this.canPlayerAccess = canPlayerAccess;
        this.markDirtyNotification = notify;
    }

    // Client side initialization
    public MarketStockContents(int size) {
        this.vendingComponentContents = new MarketStackHandler(size);
    }

...
public class StoreTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
   ...
   public StoreTileEntity() {
	   super(TileEntityTypeInit.STORE_MINER.get());
	   this.CATEGORY = "";
	   // TODO Auto-generated constructor stub
   }
   
   public StoreTileEntity(TileEntityType<?> tileEntityTypeIn, String cat) {
	   super(tileEntityTypeIn);
	   // TODO Auto-generated constructor stub
	   this.CATEGORY = cat;
	   System.out.println("CATEGORY IN -->" + cat);
	   System.out.println("MONEY IN -->" + this.currentMoney);
	...
	public Container createMenu(int windowID, PlayerInventory playerInventory, PlayerEntity player) {
		...
		return new StoreContainer(windowID, playerInventory, stockContents, inputContents, outputContents, this, tradeInStateData);
	}
	
	@Override
	public ITextComponent getDisplayName() {
		// TODO Auto-generated method stub
		return new TranslationTextComponent("container.store");
	}
		...
		if(world.isRemote){
	    	...
	    	if(this.inputContents.getStackInSlot(0).getItem().isIn(ItemTags.getCollection().get(MONEY_TAG))) {
	    		try {
	    			...
	    			System.out.println("TRENUTNO MONEY IN-->" + this.currentMoney);
 ...
 // ---- NBT Stuff ----
	...
	public void generateStockContentsFARMER() {	
        ...
        for(int i=0; i < stockContents.getSizeInventory(); i++) {
        	stockContents.getStackInSlot(i).setDisplayName(new StringTextComponent(stockContents.getStackInSlot(i).getDisplayName().getString() +" - "+ (PriceList.getPriceForItemFARMER(stockContents.getStackInSlot(i), 100)/100.00) + "$"));
        }
    }
}
